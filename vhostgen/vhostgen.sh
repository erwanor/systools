...
function SetRecord(){
  ...
  success="OK"
  if grep -q $success $config_dir/tmp.xml; then
    echo ""
...
function CheckExpect(){
  if [ -z $1 ] || [ -z $2 ] || [ -z $3 ] && [ "$1" != "CheckDirectory" ]; then
...
if [ "CheckDomain "$subdomain"" = "1" ] || [ "CheckDirectory "$serverpath"" = "1" ]; then
